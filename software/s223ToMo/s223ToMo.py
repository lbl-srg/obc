...
mbl_paths = os.environ['MODELICAPATH'].split(":")
mbl_path = None
for path in mbl_paths:
    if os.path.exists(path) and 'Buildings' in os.listdir(path):
        mbl_path = path
        break

mbl_path = os.path.join(mbl_path, 'Buildings')

# Parse conf.yml.
with open(os.path.join(mbl_path, 'Resources', 'Scripts', 'BuildingsPy', 'conf.yml'), 'r') as FH:
...
def get_experiment_attributes(model_name, conf=CONF):
    ...
    modelicapaths = os.environ['MODELICAPATH'].split(':')
    for mopath in modelicapaths:
        mos_path = os.path.join(mopath, 'Buildings', mos_path)
