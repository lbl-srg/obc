...
block DisIntPI "Discrete integrator PI controller"
  //  fixme: check gain values at initialization
  ...
  Buildings.Controls.OBC.CDL.Continuous.Gain proGain(k=k_p)
    "Proportional gain"
...
equation
  ...
  connect(y, limiter.y)
    annotation (Line(points={{150,0},{121,0}},color={0,0,127}));
  ...
  connect(limiter.u, addPI.y)
    annotation (Line(points={{98,0},{81,0}}, color={0,0,127}));
  connect(error.y, intGain.u)
    annotation (Line(points={{-59,-60},{-50,-60},{-50,-30},{-42,-30}},color={0,0,127}));
  connect(intGain.y, sam.u)
    annotation (
  ...
  connect(zerOrdHol.y, intErrSum.u1)
    annotation (Line(points={{121,100},{130,100},{130,130},{-10, 130},{-10,116},{-2,116}},
    color={0,0,127}));
  ...
  connect(or2.y, booToRea1.u)
    annotation (Line(points={{-79,60},{-62,60}}, color={255,0,255}));
  connect(holdIntError, or2.u1)
  ...
  connect(zerOrdHol.u, limiter1.y)
    annotation (Line(points={{98,100},{91,100}}, color={0,0,127}));
  annotation (
    defaultComponentName = "disIntPI",
    Icon(graphics={
        Rectangle(
...
<p>
This block is a PI controller with a discrete timestep integral error. The 
proportional gain is calculated continuously, whereas the integral error
is calculated at <code>interval<\code> timesteps. It features an integrator 
error enable/disable parameter (<code>intErrSta<\code>) and a <code>holdIntError<\code> 
parameter to determine wheather the integral error should be cumulated or reseted 
to 0 when <code>intErrSta<\code> changes to false.
</p>

</html>", revisions="<html>
<ul>
<li>
April 15, 2018, by Milica Grahovac:<br/>
