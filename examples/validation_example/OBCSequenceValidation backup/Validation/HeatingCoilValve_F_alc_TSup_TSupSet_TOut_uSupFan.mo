...
model HeatingCoilValve_F_alc_TSup_TSupSet_TOut_uSupFan
  "Validation model for the heating coil valve control sequence with all temperatures in F and an ALC controller"
...
// Tests disable if supply fan is off
  ...
  HeatingCoilValve_F_alc heaValSta_F_alc(genEna=false, reverseAction=true)
    "Heating coil controll sequence as implemented in LBNL 33-AHU-02 (Roof)"
    annotation (Placement(transformation(extent={{-40,80},{-20,100}})));

// Tests disable if it is warm outside
  HeatingCoilValve_F_alc heaValSta_F_alc1(genEna=false)
    "Heating coil controll sequence as implemented in LBNL 33-AHU-02 (Roof)"
...
// Tests controler normal operation when supply air temperature is above limiter values
  HeatingCoilValve_F_alc
                     heaValSta_F_alc2(
    genEna=false, reverseAction=true)
              "Heating coil controll sequence as implemented in LBNL 33-AHU-02 (Roof)"
...
// Tests controler operation when supply air temperature is within limiter values
  Buildings.Controls.OBC.CDL.Continuous.Sources.Ramp uTSup2(
    ...
    startTime=0,
    height=4,
    offset=TSupSet - 4/2) "\"Supply air temperature\""
...
equation
  connect(heaValSta_F_alc.TOut, uTOutBelowCutoff.y)
    annotation (Line(points={{-41,93},{-70,93},{-70,50},{-99,50}}, color={0,0,127}));
  connect(uSupFan.y, heaValSta_F_alc.uSupFan)
    annotation (Line(points={{-59,30},{-50,30},{-50,85},{-41,85}}, color={255,0,255}));
  connect(heaValSta_F_alc1.TOut, uTOutAboveCutoff.y)
    annotation (Line(points={{-41,-27},{-70,-27},{-70,-70},{-99,-70}}, color={0,0,127}));
  connect(uSupFan1.y, heaValSta_F_alc1.uSupFan)
    annotation (Line(points={{-59,-92},{-50,-92},{-50,-35},{-41,-35}}, color={255,0,255}));
  connect(heaValSta_F_alc1.TSupSet, uTSupSet1.y) annotation (Line(points={{-41,-23},{-90,-23},{-90,
          -40},{-130,-40},{-130,-70},{-139,-70}}, color={0,0,127}));
  connect(heaValSta_F_alc1.TSup, uTSup1.y)
    annotation (Line(points={{-41,-20},{-120,-20},{-120,-28},{-139,-28}}, color={0,0,127}));
  connect(heaValSta_F_alc.TSupSet, uTSupSet.y) annotation (Line(points={{-41,97},{-90,97},{-90,80},
          {-130,80},{-130,50},{-139,50}}, color={0,0,127}));
  connect(heaValSta_F_alc.TSup, uTSup.y)
    annotation (Line(points={{-41,100},{-120,100},{-120,90},{-139,90}}, color={0,0,127}));
  connect(heaValSta_F_alc2.TOut, uTOutBelowCutoff2.y) annotation (Line(points={{139,93},{120,93},{
          120,94},{100,94},{100,50},{81,50}}, color={0,0,127}));
  connect(uSupFan2.y, heaValSta_F_alc2.uSupFan)
    annotation (Line(points={{121,30},{130,30},{130,85},{139,85}}, color={255,0,255}));
  connect(heaValSta_F_alc2.TSupSet, uTSupSet2.y) annotation (Line(points={{139,97},{108,97},{108,98},
          {76,98},{76,80},{50,80},{50,50},{41,50}}, color={0,0,127}));
  connect(heaValSta_F_alc2.TSup, uTSup2.y)
    annotation (Line(points={{139,100},{60,100},{60,90},{41,90}}, color={0,0,127}));
  connect(heaValSta_F_alc3.TOut, uTOutBelowCutoff1.y)
    annotation (Line(points={{139,-27},{110,-27},{110,-70},{81,-70}}, color={0,0,127}));
  connect(uSupFan3.y, heaValSta_F_alc3.uSupFan)
    annotation (Line(points={{121,-90},{130,-90},{130,-35},{139,-35}}, color={255,0,255}));
  connect(heaValSta_F_alc3.TSupSet, uTSupSet3.y) annotation (Line(points={{139,-23},{90,-23},{90,-40},
          {50,-40},{50,-70},{41,-70}}, color={0,0,127}));
  connect(heaValSta_F_alc3.TSup, uTSup3.y)
    annotation (Line(points={{139,-20},{60,-20},{60,-30},{41,-30}}, color={0,0,127}));
annotation (experiment(StopTime=3600.0, Tolerance=1e-06),
  __Dymola_Commands(file="HeatingCoilValve_F_alc_TSup_TSupSet_TOut_uSupFan.mos"
    "Simulate and plot"),
    Documentation(
    info="<html>
<p>
...
end HeatingCoilValve_F_alc_TSup_TSupSet_TOut_uSupFan;
