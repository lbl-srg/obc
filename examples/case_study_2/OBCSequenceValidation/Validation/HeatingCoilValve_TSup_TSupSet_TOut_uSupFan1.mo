...
model HeatingCoilValve_TSup_TSupSet_TOut_uSupFan1
  ...
  parameter Real TSup(
    final unit="K",
    final quantity = "ThermodynamicTemperature") = 288.71
    "Supply air temperature";

  parameter Real TSupSet(
    final unit="K",
    final quantity = "ThermodynamicTemperature") = 294.26
    "Supply air temperature setpoint";

  parameter Real TSatMinLowLim(
    final unit="K",
    final quantity = "ThermodynamicTemperature") = 277.59
  ...
  parameter Real LowTSupSet(
    final unit="K",
    final quantity = "ThermodynamicTemperature") = 279.26
    "Fictive low supply air temeprature setpoint to check the limiter functionality"
    annotation(Evaluate=true);

// Tests disable if supply fan is off
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant uTOutBelowCutoff(final k=TOutHeaCut - 2.78)
  ...
  HeatingCoilValve                       heaValSta(genEna=false, revAct=false)
    "Heating coil controll sequence as implemented in LBNL 33-AHU-02 (Roof)"
    annotation (Placement(transformation(extent={{-40,80},{-20,100}})));

// Tests disable if it is warm outside
  HeatingCoilValve heaValSta1(genEna=false, revAct=false)
    "Heating coil controll sequence as implemented in LBNL 33-AHU-02 (Roof)"
    annotation (Placement(transformation(extent={{-40,-40},{-20,-20}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant uTOutAboveCutoff1(final k=TOutHeaCut + 2.78)
...
// Tests controler normal operation when supply air temperature is above limiter values
  ...
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant uTOutBelowCutoff2(final k=TOutHeaCut - 2.78)
    "Outdoor air temperature is below the cutoff"
...
// Tests controler operation when supply air temperature is within limiter values
  ...
  HeatingCoilValve heaValSta3(genEna=false, revAct=false)
    "Heating coil controll sequence as implemented in LBNL 33-AHU-02 (Roof)"
    annotation (Placement(transformation(extent={{140,-40},{160,-20}})));
  Buildings.Controls.OBC.CDL.Continuous.Sources.Constant uTOutBelowCutoff1(final k=TOutHeaCut - 2.78)
    "Outdoor air temperature is below the cutoff"
...
First implementation.
    ...
    Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-180,-120},{180,120}}),
        ...
        Text(
          ...
          lineColor={0,0,127},
          textString="Normal operation above the lower limit TSup range.")}));
end HeatingCoilValve_TSup_TSupSet_TOut_uSupFan1;
