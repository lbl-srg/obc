within SystemModel.BoilerPlant.Submodels.BoilerPlantControls;
model Controller
  ...
  parameter Boolean have_priSecTemSen=false
    ...
    annotation (Dialog(tab="Primary pump control parameters",
      ...
      enable=speConTypPri == SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Types.PrimaryPumpSpeedControlTypes.temperature));
  ...
  parameter Real maxLocDpPri(
    ...
    annotation (Dialog(tab="Primary pump control parameters", group="DP-based speed regulation",
      enable=speConTypPri == SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Types.PrimaryPumpSpeedControlTypes.localDP
           or speConTypPri == SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Types.PrimaryPumpSpeedControlTypes.remoteDP));
  ...
  parameter Real maxLocDpSec(
    ...
    annotation (Dialog(tab="Secondary pump control parameters", group="DP-based speed regulation",
      enable=speConTypSec == SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Types.PrimaryPumpSpeedControlTypes.localDP));
...
protected
  ...
  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Staging.Processes.Up upProCon(
    final have_priOnl=have_priOnl,
    ...
    final relFloDif=sigDif) "Stage-up process controller"
    annotation (Placement(transformation(extent={{120,76},{140,116}})));

  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Staging.Processes.Down dowProCon(
    final have_priOnl=have_priOnl,
    final have_heaPriPum=have_heaPriPum,
    final nBoi=nBoi,
    final nSta=nSta,
    ...
    final boiChaProOnTim=boiChaProOnTim,
    final delBoiEna=delBoiEna,
    relFloDif=sigDif) "Stage-down process controller"
    annotation (Placement(transformation(extent={{120,20},{140,60}})));

  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Pumps.PrimaryPumps.Controller
    priPumCon(
    final have_heaPriPum=have_heaPriPum,
    final have_priOnl=have_priOnl,
    ...
    final Ti=Ti_priPum,
    final Td=Td_priPum,
    final speConTyp=speConTypPri) "Primary pump controller"
    annotation (Placement(transformation(extent={{120,-208},{140,-152}})));

  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.BypassValve.BypassValvePosition
    bypValPos(
    nPum=nPumPri,
    final k=k_bypVal,
    final Ti=Ti_bypVal,
    final Td=Td_bypVal) if have_priOnl "Bypass valve controller"
    annotation (Placement(transformation(extent={{120,-50},{140,-30}})));

  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.SetPoints.HotWaterSupplyTemperatureReset
    hotWatSupTemRes(
    final nPum=nPumPri,
    final nSta=nSta,
    final nBoi=nBoi,
  ...
  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.SetPoints.CondensationControl
    conSet(
    final have_priOnl=have_priOnl,
    final have_varPriPum=have_varPriPum,
    final nSta=nSta,
  ...
  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Generic.PlantDisable plaDis(
    ...
    final delBoiDis=delBoiEna) "Plant disable process controller"
    annotation (Placement(transformation(extent={{240,60},{260,80}})));

  SystemModel.BoilerPlant.Submodels.BoilerPlantControls.Pumps.SecondaryPumps.Controller
    secPumCon(
